...
class TestUploadAssetValidation:
    ...
    def test_valid_png_accepted(self, isolated_storage: Path) -> None:
        result = workspace.upload_asset(VALID_PNG_B64, "logo.png")
        assert result["filename"] == "logo.png"
        assert Path(result["path"]).exists()

    def test_valid_jpg_accepted(self, isolated_storage: Path) -> None:
        result = workspace.upload_asset(VALID_JPG_B64, "photo.jpg")
        assert result["filename"] == "photo.jpg"
        assert Path(result["path"]).exists()

    def test_truncated_png_rejected(self, isolated_storage: Path) -> None:
    ...
    def test_corrupt_deflate_png_rejected(self, isolated_storage: Path) -> None:
    ...
    def test_garbage_bytes_rejected_for_image_ext(self, isolated_storage: Path) -> None:
        garbage = base64.b64encode(b"not an image at all").decode()
        with pytest.raises(ValueError, match="image validation"):
            workspace.upload_asset(garbage, "fake.png")

    def test_corrupt_bytes_do_not_reach_disk(self, isolated_storage: Path) -> None:
    ...
    def test_valid_svg_accepted(self, isolated_storage: Path) -> None:
    ...
    def test_malformed_svg_rejected(self, isolated_storage: Path) -> None:
    ...
    def test_non_image_extension_passes_through(self, isolated_storage: Path) -> None:
